    ...
    def doTraining(self, epoch_ndx, train_dl):
        ...
        for batch_ndx, batch_tup in batch_iter:
            ...
            loss_var = self.computeBatchLoss(
                batch_ndx, batch_tup, train_dl.batch_size, trnMetrics_g, device="cuda")
            loss_var.backward()
    ...
    def doValidation(self, epoch_ndx, val_dl):
        with torch.no_grad():
            valMetrics_g = torch.zeros(METRICS_SIZE, len(
                val_dl.dataset), device=self.device)  # self.device
            self.segmentation_model.eval()
            #seg_model = self.segmentation_model.to("cpu")
            # seg_model.eval()
            ...
            for batch_ndx, batch_tup in batch_iter:
                self.computeBatchLoss(
                    batch_ndx, batch_tup, val_dl.batch_size, valMetrics_g, device="cpu")

        return valMetrics_g.to('cpu')

    def computeBatchLoss(self, batch_ndx, batch_tup, batch_size, metrics_g,
                         classificationThreshold=0.5, device="cuda"):  # add last parameter
...
#         if (device == 'cpu'):
#             segmentation_model = self.segmentation_model.to("cpu")
###
